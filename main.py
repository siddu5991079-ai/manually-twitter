...
def download_latest_images(count=3):
    ...
    # 🔥 Yahan environment variable se repo ka naam utha rahe hain
    target_repo = os.environ.get("TARGET_REPO", "siddu5991079-ai/images-SPK-jajaja-3").strip()
    
    print(f"🔍 GitHub se top {count} latest images dhoond rahe hain (Prefix: '{image_prefix}')...")
    
    # 🔥 Ab API URL dynamically ban raha hai
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"❌ GitHub API Error: Code {response.status_code} on {api_url}")
            return []
    except Exception as e:
...
# ================ very very goood, bas opper yeh api k multi karty hai options ===========================================
